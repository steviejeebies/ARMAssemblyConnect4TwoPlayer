...
disp5	MOV		R0, #0				;null terminates MEM string for puts subroutine
		...
		LDR		R4, =0x1FFFF		;timer to slow down the printing of the board for animation
		MOV		R5, #0
disp6	ADD		R5, #1
		CMP		R5, R4
		BNE		disp6
...
turn3	CMP		R4, #'Y'
		...
		BL		puts
		B		turn2
turn1	LDR		R0, =choose_red
		BL		puts
turn2	BL		get							;column player wants to place piece in is in R0
...
placeN	STRB	R0, [R2]		;stores piece into BOARD index
		...
		STMFD   sp!, {R0}		;pushes current piece (Y or R) onto stack to protect it
...
;return:
;	R1 - highest streak of last dropped piece
...
;note: the following is not a subroutine, but uses LR to return to the right place in the earler parts of this subroutine
chckCm	STMFD   sp!, {R14}			
